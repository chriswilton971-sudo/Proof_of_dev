...
describe('ProofOfDev - Developer Activity Tests', () => {
  ...
  describe('Score Updates (Developer Activity Changes)', () => {
    ...
    it('Should update score after new on-chain activity', async () => {
      ...
      const signature = await createSignature(
        ...
        1, // nonce incremented
        DEADLINE,
        true // isUpdate — must sign with UPDATE_TYPEHASH to match updateScore()
      );
  ...
  describe('Token Burning', () => {
    ...
    it('Should allow re-minting after burning', async () => {
      ...
      // _nextTokenId never decrements on burn — re-minting gets a fresh,
      // higher tokenId (2), and totalSupply() tracks "highest tokenId
      // ever issued", not "currently held tokens". See ProofOfDev.sol.
      expect(await proofOfDev.balanceOf(dev1.address)).to.equal(1);
      expect(await proofOfDev.ownerOf(2)).to.equal(dev1.address);
      expect(await proofOfDev.totalSupply()).to.equal(2);
    });
  });
