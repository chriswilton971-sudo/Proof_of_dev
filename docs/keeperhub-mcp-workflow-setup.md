...
> **Note:** the app's current code (`services/analysis/keeperhub.js`) calls
> KeeperHub's **Direct Execution** API (`execute_contract_call`) instead of
> a pre-built workflow, so following this doc is optional, not required —
> the app works without it. Both are valid, documented KeeperHub paths;
> this one is kept as a reference for anyone who'd rather use a dashboard
> workflow (e.g. for branching/multi-step logic beyond a single
> `markVerified` call), or who has already validated this path separately.
> If you go this route, `keeperhub.js` would need updating to call
> `POST /api/workflows/{id}/execute` instead of Direct Execution to
> actually use the resulting workflow ID — it doesn't read
> `KEEPERHUB_MARKVERIFIED_WORKFLOW_ID` today.
