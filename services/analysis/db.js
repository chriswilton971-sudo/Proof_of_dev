 ...
 * Returns null only when the job genuinely hasn't been written yet (still
 * pending). If MongoDB itself is unreachable, returns an explicit FAILURE
 * record instead of null — in this multi-process architecture (API and
 * worker run as separate Node processes), the in-memory Map fallback is
 * process-local, so a worker's write during a Mongo outage is invisible to
 * the API process. Returning null in that case previously made api.js
 * report status "PENDING" indefinitely, even though the underlying job may
 * have already failed or succeeded with no way for this process to know.
 * @param {string} jobId
 * @returns {Promise<object|null>}
 */
export async function getJobResult(jobId) {
  ...
  if (!db) {
    return {
      ...
      stage: "unavailable",
      error:
        "Analysis service lost its database connection and cannot confirm this job's status. Please retry.",
    };
  }
  ...
  try {
    ...
    return {
      status: "FAILURE",
      stage: "unavailable",
      error: `Analysis service database error: ${err.message}`,
    };
  }
}
